...
void median_filter_(type* signal, int signal_size, int window_size)
{
	...
	for( auto ( cell ) { window }, ( median_cell ) { window + window_size / 2 };
		cell < window + window_size; cell++ )
	{
		if( cell < median_cell )
		{
			*cell = 0;
	...
	{
		auto ( next_element ) { current_element + static_cast<int>( ceil(window_size / 2.0) ) };
